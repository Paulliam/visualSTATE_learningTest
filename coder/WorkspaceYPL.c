...
extern VSActionExprFunctionType const vsactionExpr[3];
...
VSRC VSReinitialize (void)
{
  ...
  vsso.currentState[0x001U] = 0x004U;
  /*  Set top machine active.  */
  vsso.activeMachineBits[(0x001U) >> 0x003U] |= (VS_UINT8)(0x001U << ((0x001U) & 0x007U));
...
VSRC VSDeduct (VSTriggerType eventNoArg)
{
  ...
  do
  {
    ...
    if ((vsso.lastMachine <= vsso.firstMachine) || ((vssc.trans[vsso.headerTransIndex] & 0x002U) == 0U))
    {
      VSDeduct_Ols0 ();
      ...
      if ((vssc.trans[vsso.headerTransIndex] & 0x002U) != 0U)
      {
        ...
        else
        {
          ...
          vsso.firstMachine = 0x000U;
        }
      }
      /*  Set type of last fired transreaction.  */
      vsso.wasLastFiredTransreactionAnInternalReaction = (VS_BOOL)((vssc.trans[vsso.headerTransIndex] & 0x002U) == 0x000U);
...
static void VSActExprAndSignals (void)
{
  ...
  if ((vssc.trans[vsso.transitionHeaderWordIndex] & 0x001U) != 0x000U)
  {
